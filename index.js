...
function gulpShopifyUpload(apiKey, password, host, themeid, options) {
  ...
  var connectedToLogMessage = function (host, themeid, name) {
    return chalk.gray('Connected to: ') + chalk.magenta(`${host}${chalk.gray(`/?preview_theme_id=`)}${themeid}`) + chalk.gray(' theme id: ') + chalk.magenta(themeid) + chalk.gray(' theme name: ') + chalk.magenta(name);
  };
